...
main(NoRows, NoCols, Obst, Src, Dest) ->
    ...
    find_shortest_path(UpdatedA, NoRows, NoCols, Src, Dest).
...
find_shortest_path(A, NoRows, NoCols, Src, Dest) ->
    Queue = queue:new(),
    Visited = [],
    {RowI, ColI} = Src,
    UpdatedQueue = queue:in({RowI, ColI, [Src]}, Queue),
    % io:format(queue:is_empty(UpdatedQueue)),
    while_loop(A, NoRows, NoCols, Dest, UpdatedQueue, Visited).

%BFS loop for exploring the paths
while_loop(Matrix, NoRows, NoCols, Dest, Queue, Visited) ->
        ...
        false ->
            ...
            case {RowI, ColI} of
                ...
                _ ->
                    ...
                    Neighbors = get_valid_neighbors({RowI, ColI}, Matrix, NoRows, NoCols, UpdatedVisited),

                    UpdatedQueueWithNeighbors = enqueue_neighbors(Neighbors, Path, UpdatedQueue),

                    while_loop(Matrix, NoRows, NoCols, Dest, UpdatedQueueWithNeighbors, UpdatedVisited)
            end
    end.
...
get_valid_neighbors({RowI, ColI}, A, NoRows, NoCols, Visited) ->
